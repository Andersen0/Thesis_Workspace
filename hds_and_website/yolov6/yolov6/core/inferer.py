...
class Inferer(Node):
    def __init__(self, source, webcam, webcam_addr, weights, device, yaml, img_size, half):
        super().__init__('inferer_node')
        self.__dict__.update(locals())
        self.depth_cam_sub = self.create_subscription(Image,'/camera/aligned_depth_to_color/image_raw',self.depth_cam_callback,10)
        self.rgb_cam_sub = self.create_subscription(Image,'/camera/color/image_raw',self.rgb_cam_callback,10)
        #self.depth_cam_sub = self.create_subscription(String,'/rand_string',self.depth_cam_callback,10)
        self.depth_pub = self.create_publisher(Float32, 'dist_to_detection', 10)
        self.zone_pub = self.create_publisher(String, 'detection_zone', 10)
        self.depth_img = None
        self.class_detection = self.create_publisher(String, 'class_detection', 10)
        self.image_pubber = self.create_publisher(Image, 'yolo_im', 10)
        self.cropped_pubber = self.create_publisher(Image, 'cropped_im', 10)
        self.cropped_depth_pubber = self.create_publisher(Image, 'cropped_depth_im', 10)
    ...
    def infer(self, conf_thres=0.5,
        iou_thres=0.45,
        max_det=1000,
        save_txt=True,
        save_dir=osp.join(ROOT, "src/yolov6_DATA/save_dir"),
        classes=None,
        agnostic_nms=False,
        hide_labels=False,
        hide_conf=False,
        ...
        img_src = self.rgb_img #THIS OVERWRITES THE IMAGE FROM LOAD_DATA WITH THE RGB IMAGE
        #for not_used_img_src, img_path, vid_cap in tqdm(self.files):
        img_path = "0"
        ...
        pred_img = np.zeros_like(img_ori)
        ...
        if len(det):
            ...
            pub_msg = String()
            pred_depth_img = np.zeros_like(self.depth_img)
            for *xyxy, conf, cls in reversed(det):
                xyxy_list = [int(e_.item()) for e_ in xyxy]
                if self.depth_img is not None:
                    self.depth_img = cv2.resize(self.depth_img, (img_src.shape[1], img_src.shape[0]))
                ...
                if save_txt:  # Write to file
                    ...
                    line = (cls, *xywh, conf)
                    pred_area = xywh[2]*xywh[3] 
                    #self.get_logger().info(str(xywh))

                    #Publishing classes detected
                    # NB! hardcoded string formatting here
                    #pub_msg.data = str(str(cls)[7]+str(",")+str(conf)[7:11],str(","), str(pred_area))
                ...
                if draw_predictions:
                    ...
                    if depth_int < 3000:
                        ...
                        zone_msg.data = "green"
                        zone_color = (0, 255, 0)
                    self.plot_box_and_label(img_ori, max(round(sum(img_ori.shape) / 2 * 0.003), 2), xyxy, label, color=zone_color)
                    self.zone_pub.publish(zone_msg) 
                    pub_msg.data += "{},{},{},{},{};".format(str(cls)[7], str(conf)[7:11], str(round(pred_area, 3)), str(depth.data), str(zone_msg.data))
        ...
        self.cropped_pubber.publish(self.bridge.cv2_to_imgmsg(np.asarray(pred_img), "bgr8"))
        ...
        if view_img:
            ...
            if self.depth_img is not None and len(det) > 0: 
                # Display pred_img in a window
                cv2.imshow('Image', pred_img)

                # Move the first window to the upper left corner of the screen
                cv2.moveWindow('Image', 0, 0)

                # Display pred_depth_img in a different window
                cv2.imshow('Depth Image', pred_depth_img)

                # Move the second window to a different location, e.g., right of the first window
                cv2.moveWindow('Depth Image', pred_img.shape[1] + 200, 0)
...
#                    cv2.imshow("raw_image", img_src)
    ...
    @staticmethod
    def plot_box_and_label(image, lw, box, label='', color=(128, 128, 128), txt_color=(0, 0, 0), font=cv2.FONT_HERSHEY_COMPLEX):
